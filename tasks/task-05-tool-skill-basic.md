...
- [x] 实现 `MCPToolDefinition` Pydantic 模型，完整映射 PRD 2.3.1 节 JSON Schema：
  ```python
  ...
  ```
- [x] 实现 `MCPToolLoader.from_json(path)` — 从 `configs/tools/*.json` 加载工具定义
- [x] 实现 `MCPToolLoader.from_dict(data)` — 从字典加载（运行时动态注册）

### 2. 工具注册表（`src/tools/registry/`）
- [x] 实现 `ToolRegistry` 单例类：
...
- [x] 应用启动时（FastAPI lifespan）自动调用 `load_from_dir` 完成初始注册

### 3. 执行器分层与 Docker 实现（`src/tools/docker/`）
- [x] 定义统一 `ToolExecutor` 协议与 `ToolExecutorFactory.get(sandbox_type)`（接口优先，支持后续 wasm）
- [x] 先落地一个本地/进程内执行器用于主流程联调与单测，确保 Task 02 `act` 节点可先无阻塞接入
- [x] 实现 `DockerToolExecutor` 基础版（可替换）：
...
- [x] 实现 `ToolCaller` 类，供 Runtime `act` 节点调用：
  ```python
  class ToolCaller:
      async def call(
          self,
          ...
          # 6. 返回 ToolResult
  ```
- [x] `ToolResult` 包含：`success: bool`、`output: dict | None`、`error: str | None`、`latency_ms: int`

### 5. Skill 技能管理器（`src/tools/skills/`）
- [x] 实现 `SkillLoader.load(skill_dir)` 方法，解析 `SKILL.md`：
...
### 6. 示例工具与技能定义
- [x] 创建 `configs/tools/http-request.json` — 通用 HTTP 请求工具（Mock 实现，用于测试）
- [x] 创建 `configs/tools/echo.json` — Echo 工具（原样返回输入，用于单元测试）
- [x] 创建 `configs/skills/demo-skill/SKILL.md` — 示例技能定义（参考 PRD 2.3.4 节格式）

### 7. API 接口（`src/api/routes.py` 扩展）
- [x] `GET /tools` — 列出所有已注册工具
- [x] `GET /tools/{tool_name}` — 查看工具详情（含 input/output schema）
- [x] `GET /skills` — 列出所有已加载技能
- [x] `POST /tools/{tool_name}/test` — 直接测试工具调用（Dev 环境使用）

---

## 验收标准
- [x] 扫描 `configs/tools/` 目录，所有工具定义自动注册到 Registry
- [x] 通过 `ToolCaller.call("echo", {"message": "hello"})` 成功执行并返回结果（默认执行器）
- [x] `DockerToolExecutor` 完成至少 1 条工具调用冒烟测试（可在集成测试或手工验收中完成）
- [x] `blocked_actions` 策略生效：调用被禁止的工具时返回 `PermissionError`
- [x] `configs/skills/demo-skill/SKILL.md` 加载后，Skill Prompt 正确注入 Agent System Prompt
- [x] 工具调用记录准确写入 `tool_logs` 表（含 `latency_ms`、`success`）
