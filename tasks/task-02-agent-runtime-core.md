...
- [ ] 使用 `TypedDict` 定义 `AgentState`，字段包含：
  ```python
  ...
  ```
- [ ] 实现 `build_agent_graph(agent_config)` 函数，构建并返回编译好的 LangGraph `CompiledGraph`
...
- [ ] 实现 `route_after_act(state)` 路由函数：
  - 工具执行成功 → `"reflect"`
  - 工具执行失败 / 重试耗尽 → `"reflect"`（带失败标记）
- [ ] 实现 `route_after_reflect(state)` 路由函数：
...
- [ ] 实现 `AgentConfigParser.parse(agent_md_path)` 方法：
...
- [ ] 封装 `PostgresSaver.from_conn_string()` 初始化逻辑
- [ ] 提供 `get_checkpointer()` 异步工厂函数，供 `build_agent_graph()` 调用
- [ ] 处理连接池复用，避免每次重建

### 7. API 接口（`src/api/routes.py` 扩展）
- [ ] `POST /agents` — 创建 Agent（接收 `agent_config_path` 或内联配置 JSON）
- [ ] `POST /agents/{agent_id}/run` — 启动 Agent 执行任务（`{"goal": "...", "thread_id": "..."}`）
- [ ] `GET /agents/{agent_id}/status` — 查询 Agent 状态
- [ ] `POST /agents/{agent_id}/pause` — 暂停 Agent
- [ ] `POST /agents/{agent_id}/resume` — 恢复 Agent（传入 `thread_id`）

### 8. 示例 Agent 配置（`configs/agents/`）
- [ ] 创建 `configs/agents/demo-agent.md`，参考 PRD 2.1.3 节示例格式，用于验收测试

---

## 验收标准
- [ ] 加载 `configs/agents/demo-agent.md`，成功解析配置
- [ ] `POST /agents/run` 触发 Agent 执行，能完成 ≥ 3 步工具调用（使用 Mock 工具）
- [ ] 中断后通过相同 `thread_id` 恢复，执行上下文完整保留
- [ ] `step_count` 达到 `max_steps` 时，Agent 自动终止，状态置为 `failed`
- [ ] 所有工具调用记录写入 `tool_logs` 表
